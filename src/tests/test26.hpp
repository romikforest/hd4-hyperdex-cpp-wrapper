#ifndef TEST26_HPP_INCLUDED
#define TEST26_HPP_INCLUDED
...
// File: test26.hpp
// First created: 2015-10-24-11.17
//
...
void test26()
{
  std::cout << "Test 26: Admin. seria start-end." << std::endl;
  ...
  res = admin.seria_start();

  if (!res) { LOG(error) << "Seria can't be started."; return; }

  auto ans = admin.add_space(description);
...
//  if (admin.loop_all())
//             LOG(info)  << "Space added.";
//  else     { LOG(error) << "Space is not added."; return; }

  res = admin.seria_end();

  if (!res) { LOG(error) << "Seria can't be ended. Space is not added."; return; }
  LOG(info)  << "Space added.";
